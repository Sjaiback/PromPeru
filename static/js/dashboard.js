(function () {
  ...
  function doughnut(id, set, colors) {
    var el = document.getElementById(id); if (!el) return;
    new Chart(el, { type:"doughnut", data:{ labels:labels(set.labels), datasets:[{ data:safe(set.values), backgroundColor:colors, borderColor:paper, borderWidth:5, hoverOffset:8 }] }, options:{ cutout:"69%", maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:tooltip() } } });
  }
  ...
  function drawTrend(period) {
    ...
    trendChart = new Chart(el, { type:"line", data:{ labels:labels(serie.labels).map(function(value){ return value === "Sin registros" ? value : formatTrendLabel(value,period); }), datasets:[{ data:safe(serie.values), borderColor:red, backgroundColor:gradient, fill:true, tension:.42, borderWidth:3, pointRadius:3, pointHoverRadius:7, pointBackgroundColor:"#fff", pointBorderColor:red, pointBorderWidth:3 }] }, options:{ maintainAspectRatio:false, interaction:{ mode:"index", intersect:false }, plugins:{ legend:{display:false}, tooltip:tooltip() }, scales:{ x:axis(), y:{ ...axis(), beginAtZero:true, ticks:{precision:0,stepSize:1} } } } });
  }
  drawTrend("dia");
